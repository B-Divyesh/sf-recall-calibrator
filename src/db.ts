...
function createTransaction(name: string) {
  return async function transaction<T>(storeName: typeof STORES[number], mode: IDBTransactionMode, action: (store: IDBObjectStore) => IDBRequest<T>): Promise<T> {
...
function createWriteAll(name: string) {
  return async function writeAll(action: (stores: Record<typeof STORES[number], IDBObjectStore>, tx: IDBTransaction) => void, failureMessage: string): Promise<void> {
...
export function createDatabase(name = DB_NAME) {
  ...
  const writeAll = createWriteAll(name);
  return {
...
/** The ordinary, private database. Demo mode always creates its own instance. */
